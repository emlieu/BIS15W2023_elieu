...
```{r}
homerange %>% 
  ggplot(aes(x= class, y= log10.mass, group_by = taxon))+
  geom_boxplot()
```
...
#.groups = 'keep' keeps year as a grouping variable
```

```{r}
deserts2 <- deserts %>% mutate(year=as_factor(year)) #so you can see every year
```
...
homerange %>% 
  ...
  geom_histogram(color = "pink", fill = "lavenderblush2", bins=40)+
...
```{r}
homerange %>% 
  ggplot(aes(x= log10.hra, fill= "violetred", color = "black", bins=10))+
  geom_histogram()+
  labs(title= "Log10.hra vs Count")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y = after_stat(density)), fill= "lavenderblush", alpha = .4, color = "purple")+
  geom_density(color= "orange")+
...
rhomerange %>% 
  ...
  geom_bar(position="dodge", color= "black") #position="dodge" makes the bar graph not stacked
  labs(title = "Observations by Taxon and Range Category in Homerange Data",
       x = "Range Category",
       fill = "Class")
```
