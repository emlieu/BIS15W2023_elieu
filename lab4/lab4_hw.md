...
```r
homerange <- select_all(homerange, tolower)
select_all(homerange, ~str_replace(., " ", "_"))
```
...
```
The largest deer is the alces alces (moose). 
...
```r
snake_mass <- select(snake, "mean.hra.m2", "common.name")
snake_mass
```

```
## # A tibble: 41 × 2
##    mean.hra.m2 common.name          
##          <dbl> <chr>                
##  1         700 western worm snake   
##  2         253 eastern worm snake   
##  3      151000 racer                
##  4      114500 yellow bellied racer 
##  5        6476 ringneck snake       
##  6     1853000 eastern indigo snake 
##  7      150600 great plains ratsnake
##  8       46000 western ratsnake     
##  9      516375 hognose snake        
## 10      110900 European whipsnake   
## # … with 31 more rows
```

```r
arrange(snake_mass, mean.hra.m2)
```
...
##    mean.hra.m2 common.name         
##          <dbl> <chr>               
##  1        200  namaqua dwarf adder 
##  2        253  eastern worm snake  
##  3        600  butlers garter snake
##  4        700  western worm snake  
##  5       2400  snubnosed viper     
##  6       2614. chinese pit viper   
##  7       6476  ringneck snake      
##  8      10655  cottonmouth         
##  9      15400  redbacked ratsnake  
## 10      17400  gopher snake        
## # … with 31 more rows
```
The namaqua dwarf adder, or bitis schneideri, has the smallest homerange. It lives in the coastal sand dunes in Namibia with a very high mortality rate due to its small size and predator-rich environment. A fun fact is that there is no antivenom for this species. 
