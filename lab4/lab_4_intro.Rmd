...
A data frame is a more appealing, organized way to display the same information as one would with a data matrix. A data frame gets its information from imported .csv files while a data matrix is constructed using previously established vectors.

2. What is the command we use to import .csv files into R?  
```{r}
readr::read_csv("filename.csv")
```
