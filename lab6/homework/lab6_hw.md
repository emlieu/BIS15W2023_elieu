...
```
There are lots of NAs and most of the variables are characters. 
...
```r
fisheries_tidy %>% 
  summarize(n_genera=n_distinct(country))
```
...
## #   ⁶​fao_major_fishing_area
```
Morroco caught the most sardines between 1990 and 2000. 
...
```r
fisheries_tidy %>% 
  filter(asfis_species_name == "Aristeus antennatus" & between(year, 1990, 2012)) %>% 
  slice_max(catch)
```
...
##   <fct>    <chr>     <fct>   <chr>   <fct>   <chr>   <fct>   <chr>   <dbl> <dbl>
## 1 Portugal Blue and… 45      Shrimp… 228020… Ariste… 27      Quanti…  2002    97
## # … with abbreviated variable names ¹​common_name, ²​isscaap_group_number,
## #   ³​isscaap_taxonomic_group, ⁴​asfis_species_number, ⁵​asfis_species_name,
## #   ⁶​fao_major_fishing_area
```
Portugal caught the most shrimpies between 1990 and 2012. yummalicious!
