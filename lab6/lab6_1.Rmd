...
msleep %>% 
  summarize(mean_sleep_total = mean(sleep_total), 
              ...
              max__sleep_total = max(sleep_total),
            total = n())
```
...
NA designates missing information and is being ignored by the summarize function; for some carnivores, for example, the brain weight was not measured, thus not alowing us to have a mean brain weight of all carnivores. 
...
```
no recorded brain weight for cetacea. 
