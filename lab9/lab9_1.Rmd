...
homerange %>% 
  ...
  select(common.name, log10.mass) %>% #select(x, y)
  ggplot(aes(x=common.name, y=log10.mass))+ #notice that one of the (x,y) values is categorical (a trait not a number or interger)
  geom_col() #geom_col lets us put in an (x,y) unlike geom_bar
```
